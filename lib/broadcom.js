...
function isLikelyEnglishUrl(url) {
  try {
    ...
    // Reject localized URL slugs when a non-Latin script is visible.
...
function isEnglishRecord(record) {
  ...
  const title = normalizeLanguageText(record.title);
  const body = normalizeLanguageText(record.text);

  if (!title) return false;

  // A title containing Japanese/Chinese/Korean/Cyrillic is not treated as
  // an English KB article, regardless of the HTML lang attribute.
  if (containsLocalizedScript(title)) {
    return false;
  }

  // Broken percent-encoded localized slug.
...
function isEnglishPage(html, title, body, url) {
  if (!isLikelyEnglishUrl(url)) {
    return false;
  }

  const cleanTitle = normalizeLanguageText(title);
  const cleanBody = normalizeLanguageText(body);
  ...
  // IMPORTANT:
  // Broadcom localized pages can still declare <html lang="en">.
  // Therefore the lang attribute is informational only and is NOT used
  // to accept a page.
  ...
  // Reject a localized article by inspecting the actual visible content.
  return hasEnglishDominantContent(
...
function hasEnglishDominantContent(text) {
  ...
  const japanese =
    (sample.match(/[\u3040-\u30ff\u31f0-\u31ff]/gu) || []).length;

  const cjk =
    (sample.match(/[\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff]/gu) || []).length;

  const korean =
    (sample.match(/[\uac00-\ud7af]/gu) || []).length;

  const cyrillic =
    (sample.match(/[\u0400-\u04ff]/gu) || []).length;
  ...
  // No alphabetic content: keep neutral technical pages rather than discard.
  if (letters === 0) {
    return true;
  }

  const localizedRatio = localized / letters;
  const latinRatio = latin / letters;

  // Strong rejection for clearly localized pages.
  if (localized >= 12 && localizedRatio >= 0.08) {
    return false;
  }

  // Even a smaller amount is rejected if non-Latin text dominates.
  if (localized >= 5 && localizedRatio >= 0.20) {
    return false;
  }

  // Require English/Latin to be clearly dominant when enough letters exist.
  if (letters >= 80 && latinRatio < 0.85) {
    return false;
  }
