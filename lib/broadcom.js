...
export async function buildBroadcomIndex(log = () => {}, options = {}) {
  const forceFull = !!options.forceFull;
  const previous = forceFull ? null : await safeReadJson(BROADCOM_INDEX_PATH);

  const previousByUrl = new Map(
    (previous?.records || []).map(v => [v.url, v])
  );

  const sitemapItems = await collectArticleItems(log);
  ...
  for (const url of extras) {
    ...
    if (c && !sitemapItems.some(v => v.url === c)) {
      sitemapItems.push({
        url: c,
        lastmod: null
      });
  ...
  const items = sitemapItems.slice(0, maxPages);

  const concurrency = clampEnv("BROADCOM_CONCURRENCY", 1, 8, 3);
  const batchSize = clampEnv("BROADCOM_BATCH_SIZE", 1, 100, 20);
  ...
  log("BROADCOM_PLAN", {
    discovered: sitemapItems.length,
    selected: items.length,
    previous: previous?.records?.length || 0,
    concurrency,
    batchSize,
    forceFull
  });
  ...
  for (let start = 0; start < items.length; start += batchSize) {
    ...
    for (let i = 0; i < batch.length; i += concurrency) {
      ...
      const groupResults = await Promise.all(
        group.map(async item => {
          ...
          // Incremental reuse:
          // If sitemap provides lastmod and it did not change, reuse the body.
          if (
            !forceFull &&
            prev &&
            item.lastmod &&
          ...
          // If sitemap has no lastmod, keep previous record unless a periodic
          // age threshold says it should be refreshed.
          if (!forceFull && prev && !item.lastmod) {
          ...
          try {
            ...
            // Keep old record if update failed.
            if (prev) return prev;
      ...
      records.push(...groupResults);
    }

    const processed = Math.min(start + batch.length, items.length);

    log("BROADCOM_PROGRESS", {
      processed,
      total: items.length,
      fetched,
      reused,
    ...
    await writeJsonBlob(BROADCOM_STATE_PATH, {
      status: "running",
      processed,
      total: items.length,
      fetched,
...
async function collectFromSitemapXml(
  ...
  // Fallback for simple XML containing only <loc>.
...
function extractArticleBody(html) {
  ...
  // Prefer likely article-content containers.
  const candidates = [
...
async function fetchText(url) {
  const response = await fetch(url, {
    headers: {
      "User-Agent":
        "Mozilla/5.0 (compatible; JandiNotionBroadcomSearch/2.0)"
    },
    redirect: "follow"
  });
