...
export const INDEX_PATH = "jandi-notion/notion-index.json";
...
export async function notionFetch(token, path, options = {}) {
  ...
  throw new Error(`NOTION_RATE_LIMIT_HTTP_${lastResponse?.status || 429}`);
}
...
export function blockToText(block) {
  ...
  if (data && Array.isArray(data.rich_text)) {
    ...
    if (Array.isArray(data.caption)) {
      parts.push(richTextToPlain(data.caption));
    }
